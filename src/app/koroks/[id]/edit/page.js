...
export default function EditKorokForm({searchParams}){
    const [userId, setUserId]=useState(null)
    const [userName, setUserName]=useState(null)
    const [formData, setFormData]=useState({...searchParams})
    const [imagePreviews, setImagePreviews]=useState({
        locationImage: formData.locationImage,
        korokImage: formData.korokImage
    })
    ...
    const editKorok = async (korok) => {
        const response= await fetch(`https://hyrule-archive.herokuapp.com/koroks/${searchParams._id}`,
        {
            method: "PUT",
            mode: "cors",
            // headers: {
    ...
    return (<>
            <h1 className={styles.title}>Edit Korok</h1>
            {userId!=null ?
            <div className={styles.formContainer}>
                <form className={styles.form}>
                    ...
                    <div className={styles.textInputs}>
                        ...
                        <textarea name='locationDescription' placeholder='Describe the location' value={formData.locationDescription} onChange={handleChange}/><br/>
                        <textarea name='korokDescription' placeholder="Describe how to find the korok" value={formData.korokDescription} onChange={handleChange}/><br/>
                    </div>
